...
def usage():
    ...
    2) python ./extract_segments.py  
    3) python ./extract_segments.py  -p  
    """)
...
def  extract_code_segment_hex_from_diassembly_file(dir_path, disassemble_file,seg_name_str,start_addr_str, stop_addr_str, enable_padded):
...
def main():
    try:
        opts,args = getopt.getopt(sys.argv[1:],'hpi:o:v',["help","padded" "input=", "output=","verbose"])
    ...
    for  o, a in opts: 
        if o == "-v":
            ...
            option_padded =  True 
            print("enable padded for discontinue address")
        else:
    ...
    #extract_code_segment_hex_from_diassembly_file(map_file_path, disassemble_file, "POA","0x0033a8","0x009810")
    #extract_code_segment_hex_from_diassembly_file(map_file_path, disassemble_file, "POB","0x600033a8","0x60003606")
    #extract_code_segment_hex_from_diassembly_file(map_file_path, disassemble_file, "POD","0x000200","0x003399")
    #extract_rodata_segment_hex_from_diassembly_file(map_file_path, disassemble_file, "ROA","0x122f08","0x127850")
    #extract_rodata_segment_hex_from_diassembly_file(map_file_path, disassemble_file, "ROB","0x60122f08","0x6012c4a0")
    #extract_rodata_segment_hex_from_diassembly_file(map_file_path, disassemble_file, "ROD","0x120000","0x122f00")
    ...
    for seg_name, items in groupby(segments_map_list_merge, key=itemgetter('seg_name')):
        ...
        if (str(seg_name).startswith("P")): 
            ...
            for i in items:
                ...
                extract_code_segment_hex_from_diassembly_file(map_file_path, disassemble_file, i["seg_name"],s_seg_start,s_seg_stop, option_padded)
